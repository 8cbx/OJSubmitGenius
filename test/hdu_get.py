...
class DealWithPages(HTMLParser.HTMLParser):
	...
	def handle_starttag(self,tag,attrs):
		...
		if tag == 'sup':
			flag = 2
		if stack_size == 1 and tag == 'hi':
			picin = 1
			self.st.push('h1')
		if stack_size == 1 and tag == 'div':
			for name,value in attrs:
				if name == 'class' and value == 'panel_title':
					picin = 1
					self.st.push('div')
					break
		if stack_size == 1 and tag == 'div':
			for name,value in attrs:
				if name == 'class' and value == 'panel_content':
					picin = 1
					self.st.push('div')
					break
	...
	def handle_endtag(self, tag):
		...
		if 'div' == tag and 'div' == stack_tag:
			self.st.pop()
			picin = 0
		if 'h1' == tag and 'h1' == stack_tag:
			self.st.pop()
			picin = 0

if __name__ == '__main__':
	for num in range(1012,1013):
		page = getPageContent('http://acm.hdu.edu.cn/showproblem.php?pid='+str(num))
		#print page
		print '\n'+str(num)
		my=DealWithPages()
		my.feed(page)
